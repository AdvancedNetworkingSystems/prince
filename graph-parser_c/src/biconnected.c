                            ...
                            int *component_indexes)
{
        ...
        while (n != 0) {
                ...
                if (index[ng->node_graph_id] < 0) {
                        strongconnect(
                            ng, &current_index, &s, index, low_link, on_stack,
                            added, sg, is_articulation_point, component_indexes,
                            &component_index, 0, bcc_indexes, &bcc_index);
                }
              ...
              int *bcc_num)
{
        ...
        while (true) {
                if (iterator[last->node_graph_id] != 0) {
                        struct node_graph *w =
                            ((struct edge_graph *)iterator[last->node_graph_id]
                                 ->content)
                                ->to;
                ...
                for (edge_iterator = u->neighbours.head; edge_iterator != 0;
                     edge_iterator = edge_iterator->next) {
                        ...
                        if (!visited[v->node_graph_id]) {
                                ...
                                low[u->node_graph_id] = min(
                                    low[u->node_graph_id], d[v->node_graph_id]);
                        }
                }
        }
}
