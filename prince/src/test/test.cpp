...
c_graph_parser *cg;
extern "C" {
void init(int h) { cg = new_graph_parser(1, h); }
void parse(char *g)
{
        struct topology *t = parse_netjson(g);
        graph_parser_parse_simplegraph(cg, t);
}
void compute() { graph_parser_calculate_bc(cg); }
PyObject *get_res()
{
        ...
        for (i = 0; i < map.size; i++) {
                ...
                PyDict_SetItem(d, PyString_FromString(map.map[i].id),
                               PyFloat_FromDouble(map.map[i].bc));
        }
        return d;
}
void destroy() { delete_graph_parser(cg); }
}
