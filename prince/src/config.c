		...
		if (strcmp(key, "proto") == 0) {
			if (json_object_get_type(val) == json_type_object) {
				json_object_object_foreach(val, key_i, val_i)
				{
					if (strcmp(key_i, "protocol") == 0) {
					...
					} else if (strcmp(key_i, "json_type") == 0) {
						if (json_object_get_type(val_i)
						    == json_type_string) {
							const char *content =
								json_object_get_string(val_i);
							if (strcmp(content, "netjson") == 0) {
								ph->json_type = 1;
							} else if (strcmp(content, "jsoninfo")
								   == 0) {
								ph->json_type = 0;
							} else {
								fprintf(stderr,
						...
						} else if (json_object_get_type(val_i)
							   == json_type_int) {
